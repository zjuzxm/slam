...
TotalCloud = {zeros(1,num(1,1) )};
j = 1;
for i = 1 : 10 : n
    file_path = strcat(dir_path,int2str(i));
    file_path = strcat(file_path,'.pcd');
    ptCloud = pcread(file_path);
    Denoise_Cloud = pcdenoise(ptCloud);
    TotalCloud{j} = Denoise_Cloud;
    j = j + 1;
    fprintf('Cloud %d \n',i) ;
end

ptCloudRef = TotalCloud{1};
ptCloudCurrent = TotalCloud{2};
...
for i = 3:length(TotalCloud)
    fprintf('icp %d \n',i) ;
    ptCloudCurrent = TotalCloud{i};
       
    % Use previous moving point cloud as reference.
    moving = pcdenoise(moving);
    fixed = pcdownsample(moving,'gridAverage', gridSize);
    ...
    hScatter.CData = ptCloudScene.Location(:,1);
    drawnow('limitrate')
end
